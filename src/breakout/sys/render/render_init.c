...
void m_breakout_sys_render_bg() {    
    for (u16 y = 0; y < ERS_SCREEN_H; y += G_SPR_BRK_BG_0_H) {
        for (u16 x = 0; x < ERS_SCREEN_W; x += ERS_BYTES_TO_PX(G_SPR_BRK_BG_0_W)) {
            if (y == 0 && x > 0) {
                cpct_drawSprite(
        ...
        if (y > 0) {
            ...
            cpct_drawSprite(
                g_spr_brk_bg_4, ers_get_screen_ptr(ERS_SCREEN_W - ERS_BYTES_TO_PX(G_SPR_BRK_BG_0_W), y), G_SPR_BRK_BG_0_W, G_SPR_BRK_BG_0_H
            );
        }
    }
    cpct_drawSprite(
