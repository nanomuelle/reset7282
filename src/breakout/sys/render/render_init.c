...
void _breakout_sys_render_bg() {
    ...
    for (u8 row = ROOM_TXT_BOX_H / G_SPRITE_BREAKOUT_BG_H; row > 0; --row) {
        ...
        for (u8 col = ROOM_TXT_BOX_W / G_SPRITE_BREAKOUT_BG_W; col > 0; --col) {
            cpct_drawSprite(g_sprite_breakout_bg, pmem, G_SPRITE_BREAKOUT_BG_W, G_SPRITE_BREAKOUT_BG_H);
            pmem += G_SPRITE_BREAKOUT_BG_W;
        }

        y += G_SPRITE_BREAKOUT_BG_H;
    }
