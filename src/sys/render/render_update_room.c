...
void _update_roomName(TRoom *room) {
    ...
    _render_clearBox(ROOM_NAME_X, ROOM_NAME_Y, ROOM_NAME_W, LINE_HEIGHT, color);
...
void _update_roomOuts(TRoom *room) {
    ...
    _render_clearBox( OUTS_X, OUTS_Y, 11, LINE_HEIGHT, color);
...
void _update_roomObjs(TRoom *room) {
    ...
    // lightbulb
    u8* lightbulb = man_rooms_isDark(room) ? g_spr_lightbulb_0 : g_spr_lightbulb_1;

    cpct_drawSprite(
        lightbulb,
        cpct_getScreenPtr(CPCT_VMEM_START, 0, ROOM_TXT_BOX_Y),
        G_SPR_LIGHTBULB_0_W, G_SPR_LIGHTBULB_0_H
    );
